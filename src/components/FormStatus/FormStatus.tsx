...
export const FormStatus: FunctionComponent<FormStatusProps> = ({
  ...
  return (
    <div
      {...restProps}
      scopedClass={classNames(getClassName('FormStatus', platform), `FormStatus--${mode}`)}
    >
      {hasReactNode(header) && <Headline weight="medium" scopedClass="FormStatus__header">{header}</Headline>}
      {dangerouslySetInnerHTML &&
