...
export class PanelHeaderContext extends Component<PanelHeaderContextProps & DOMProps, PanelHeaderContextState> {
  ...
  render() {
    ...
    return (
      <FixedLayout {...restProps} scopedClass={classNames(baseClassNames, {
        'PanelHeaderContext--opened': opened,
        'PanelHeaderContext--closing': closing,
        'PanelHeaderContext--desktop': this.isDesktop,
      })} vertical="top">
        <div scopedClass="PanelHeaderContext__in" ref={this.elementRef}>
          <div scopedClass="PanelHeaderContext__content">
            {(opened || closing) && children}
          </div>
        </div>
        {!this.isDesktop && (opened || closing) && <div onClick={onClose} scopedClass="PanelHeaderContext__fade" />}
      </FixedLayout>
    );
  }
}
