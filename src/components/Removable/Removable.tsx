...
export const Removable: FC<RemovableProps> = withAdaptivity((props: RemovableProps & Pick<AdaptivityProps, 'sizeY'>) => {
  ...
  return (
    ...
    >
      <div scopedClass="Removable__content" style={platform === IOS ? { transform: `translateX(-${removeOffset}px)` } : null}>
        {children}

        {(platform === ANDROID || platform === VKCOM) &&
          <IconButton scopedClass="Removable__control Removable__remove" onClick={onRemoveClick}>
            <Icon24Cancel />
          </IconButton>
        }
        {platform === IOS && (
          <button scopedClass="Removable__control Removable__indicator" onClick={onRemoveActivateClick}>
            <i scopedClass="Removable__indicator-in" />
          </button>
        )}
      </div>
      ...
      {platform === IOS &&
        <button
          ref={removeButtonRef}
          scopedClass="Removable__remove"
          onClick={onRemoveClick}
          style={{ transform: `translateX(-${removeOffset}px)` }}
        >
          <span scopedClass="Removable__remove-in">{removePlaceholder}</span>
        </button>
      }
    </div>
  );
