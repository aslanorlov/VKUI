...
export interface AppRootProps extends HTMLAttributes<HTMLDivElement>, AdaptivityProps {
  embedded?: boolean;
  window?: Window;
  /** Добавляет на элементы классы без префикса (.Button) для обратной совместимости */
  noConflictClass?: boolean;
}
...
const AppRoot: FC<AppRootProps> = ({ children, embedded, sizeX, hasMouse, noConflictClass = false }) => {
  ...
  if (!initialized.current) {
    ...
    classScopingMode.noConflict = noConflictClass;
  }
  ...
  return (
    <div ref={rootRef} scopedClass={classNames('AppRoot', {
