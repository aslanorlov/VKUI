...
const ModalCard: FC<ModalCardProps> = (props: ModalCardProps) => {
  ...
  return (
    <div
      {...restProps}
      scopedClass={classNames(getClassName('ModalCard', platform), {
        'ModalCard--desktop': isDesktop,
      })}
    >
      <div scopedClass="ModalCard__in" ref={refs.innerElement}>
        <div scopedClass="ModalCard__container">
          {hasReactNode(icon) && <div scopedClass="ModalCard__icon">{icon}</div>}
          {hasReactNode(header) && <Title level="2" weight="semibold" scopedClass="ModalCard__header">{header}</Title>}
          {hasReactNode(subheader) && <Subhead weight="regular" scopedClass="ModalCard__subheader">{subheader}</Subhead>}

          {children}

          {hasReactNode(actions) &&
          <div scopedClass={classNames('ModalCard__actions', {
          ...
          {canShowCloseBtnIos &&
          <PanelHeaderButton scopedClass="ModalCard__dismiss" onClick={onClose || modalContext.onClose}>
            <Icon24Dismiss />
          </PanelHeaderButton>
          }
        </div>
