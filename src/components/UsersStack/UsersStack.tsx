...
const UsersStack: FC<UsersStackProps> = (props) => {
  ...
  return (
    ...
    >
      <div scopedClass="UsersStack__photos">
        {photosShown.map((photo, i) => (
          <div
            key={i}
            scopedClass="UsersStack__photo"
            style={{ backgroundImage: `url(${photo})` }}
          />
        ))}

        {canShowOthers &&
          <Caption weight="medium" level="1" scopedClass="UsersStack__photo UsersStack__photo--others">
            <span>+{othersCount}</span>
          </Caption>
        }
      </div>
      {children &&
        <Caption weight="regular" level="1" scopedClass="UsersStack__text">
          {children}
        </Caption>
      }
    </div>
