...
class Alert extends Component<AlertProps, AlertState> {
  ...
  renderHeader(header: ReactNode) {
    switch (this.props.platform) {
      case VKCOM:
        return <Headline scopedClass="Alert__header" weight="medium">{header}</Headline>;
      case IOS:
        return <Title scopedClass="Alert__header" weight="semibold" level="3">{header}</Title>;
      case ANDROID:
        return <Title scopedClass="Alert__header" weight="medium" level="2">{header}</Title>;
    }
  }
  ...
  renderText(text: ReactNode) {
    switch (this.props.platform) {
      case VKCOM:
        return <Caption scopedClass="Alert__text" level="1" weight="regular">{text}</Caption>;
      case IOS:
        return <Caption scopedClass="Alert__text" level="2" weight="regular">{text}</Caption>;
      case ANDROID:
        return <Headline scopedClass="Alert__text" weight="regular">{text}</Headline>;
    }
  }
  ...
  renderAction = (action: AlertActionInterface, i: number) => {
    ...
    switch (platform) {
      case ANDROID:
        return (
          <Button
            scopedClass={classNames('Alert__button', `Alert__button--${action.mode}`)}
            mode="tertiary"
            size="m"
      ...
      default:
        return (
          <Tappable
            Component="button"
            scopedClass={classNames('Alert__action', `Alert__action--${action.mode}`)}
            onClick={this.onItemClick(action)}
            key={`alert-action-${i}`}
          >
            {action.title}
    ...
    return (
      ...
      >
        <div
          ...
          scopedClass={classNames(getClassName('Alert', platform), {
