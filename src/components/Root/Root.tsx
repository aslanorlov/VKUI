...
class Root extends Component<RootProps & DOMProps, RootState> {
  ...
  render() {
    ...
    return (
      <div {...restProps} scopedClass={classNames(baseClassName, {
        'Root--transition': !disableAnimation && transition,
        'Root--no-motion': disableAnimation,
      })}>
        {Views.map((view: ReactElement) => {
          return (
            <div key={view.props.id} ref={(e) => this.viewNodes[view.props.id] = e} scopedClass={classNames('Root__view', {
        ...
        <AppRootPortal>
          {!!popout && <div scopedClass="Root__popout">{popout}</div>}
          {!!modal && <div scopedClass="Root__modal">{modal}</div>}
        </AppRootPortal>
      </div>
    );
  }
