...
class CustomSelect extends React.Component<CustomSelectProps, CustomSelectState> {
  ...
  render() {
    ...
    return (
      <label
        scopedClass={getClassName('CustomSelect', platform)}
        className={className}
        style={style}
        ref={this.rootRef}
      >
        ...
        <select
          ...
          onFocus={onFocus}
          value={value}
          scopedClass="CustomSelect__control"
        >
          {options.map((item) => <option key={`${item.value}`} value={item.value} />)}
        </select>
        {opened &&
        <div
          scopedClass={classNames('CustomSelect__options', `CustomSelect__options--sizeY-${sizeY}`, {
