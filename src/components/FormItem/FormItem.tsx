...
export const FormItem: FC<FormItemProps> = withAdaptivity((props: FormItemProps & Pick<AdaptivityProps, 'sizeY'>) => {
  ...
  const wrappedChildren = (
    <Fragment>
      {hasReactNode(top) && <Subhead weight="regular" scopedClass="FormItem__top">{top}</Subhead>}
      {children}
      {hasReactNode(bottom) && <Caption level="1" weight="regular" scopedClass="FormItem__bottom">{bottom}</Caption>}
    </Fragment>
  );
