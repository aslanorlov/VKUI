...
function renderHeader({ size, header }: Pick<BannerProps, 'size' | 'header'>) {
  switch (size) {
    case 's':
      return <Headline weight="medium" scopedClass="Banner__header">{header}</Headline>;
    case 'm':
      return <Title level="2" weight="medium" scopedClass="Banner__header">{header}</Title>;
  }
}
...
function renderSubheader({ size, subheader }: Pick<BannerProps, 'size' | 'subheader'>) {
  switch (size) {
    case 's':
      return <Caption level="1" weight="regular" scopedClass="Banner__subheader">{subheader}</Caption>;
    case 'm':
      return <Text weight="regular" scopedClass="Banner__subheader">{subheader}</Text>;
  }
}
...
const Banner: FunctionComponent<BannerProps> = (props: BannerProps) => {
  ...
  return (
    ...
    >
      <InnerComponent scopedClass="Banner__in">
        {mode === 'image' && background &&
        <div scopedClass="Banner__bg">
          {background}
        </div>
        }

        {before && <div scopedClass="Banner__before">{before}</div>}

        <div scopedClass="Banner__content">
          {hasReactNode(header) && renderHeader({ size, header })}
          {hasReactNode(subheader) && renderSubheader({ size, subheader })}
          {hasReactNode(text) && <Text weight="regular" scopedClass="Banner__text">{text}</Text>}
          {actions &&
          <div scopedClass="Banner__actions">{actions}</div>
          }
        </div>

        {asideMode === 'expand' &&
        <div scopedClass="Banner__expand">
          <Icon24Chevron />
        </div>
        }

        {asideMode === 'dismiss' &&
        <div scopedClass="Banner__dismiss">
          <div scopedClass="Banner__dismissIcon" onClick={onDismiss}>
