...
const TabsItem: FunctionComponent<TabsItemProps> = ({
  ...
  return (
    <Tappable
      {...restProps}
      scopedClass={classNames(getClassName('TabsItem', platform), { 'TabsItem--selected': selected })}
      activeEffectDelay={platform === IOS ? 0 : ACTIVE_EFFECT_DELAY}
    >
      <TypographyComponent scopedClass="TabsItem__in" weight="medium">{children}</TypographyComponent>
      {hasReactNode(after) && <div scopedClass="TabsItem__after">{after}</div>}
    </Tappable>
  );
};
